      ...
      {testimonials.map((testimonial, index) => (
        ...
        >
          <motion.img
            ...
            className="w-14 h-14 md:w-16 md:h-16 rounded-full border-4 cursor-pointer object-cover"
            animate={{
              borderColor: (hoveredIndex === index || hasBeenHovered[index]) ? '#C8F200' : '#333333'
            }}
            ...
            {hoveredIndex === index && (
              ...
              >
                <div className="h-24 overflow-hidden whitespace-pre-wrap text-gray-300">
                  {typedText}
                  <span className="animate-pulse text-[#C8F200]">|</span>
                </div>
                <p className="mt-2 text-right font-semibold text-[#C8F200]">{testimonial.name}</p>
                <p className="text-right text-gray-500 text-sm">{testimonial.jobtitle}</p>
                <div className="absolute left-1/2 transform -translate-x-1/2 -bottom-4">
                  <div className="w-3 h-3 bg-black border border-[#C8F200]/30 rounded-full shadow-lg"></div>
                  <div className="w-2 h-2 bg-black border border-[#C8F200]/30 rounded-full shadow-lg mt-1 ml-0.5"></div>
                  <div className="w-1 h-1 bg-black border border-[#C8F200]/30 rounded-full shadow-lg mt-1 ml-1"></div>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
