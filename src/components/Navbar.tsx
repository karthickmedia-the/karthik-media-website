  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled 
          ? 'bg-black/90 backdrop-blur-xl border-b border-[#C8F200]/10 shadow-lg shadow-black/20' 
          : 'bg-transparent'
      }`}
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            <motion.img 
              src={karthikMediaLogo} 
              alt="The Karthik Media" 
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.slice(0, 1).map((link) => (
              <Link
                key={link.name}
                to={link.url}
                className="text-white/80 hover:text-[#C8F200] transition-colors text-sm font-medium"
              >
                {link.name}
              </Link>
            ))}
                  ...
                  >
                    {services.map((service, index) => (
                      ...
                      >
                        <Link
                          to={service.url}
                          className="block px-4 py-3 text-white/80 hover:text-[#C8F200] hover:bg-[#C8F200]/10 transition-all text-sm border-b border-white/5 last:border-0"
                        >
                          {service.name}
                        </Link>
                      </motion.div>
          ...
          <button
            ...
            className="lg:hidden p-2 text-white hover:text-[#C8F200] transition-colors"
          >
              ...
              <div className="py-4 space-y-2">
                {navLinks.slice(0, 1).map((link) => (
                  <Link
                    key={link.name}
                    to={link.url}
                    className="block px-4 py-3 text-white/80 hover:text-[#C8F200] hover:bg-[#C8F200]/5 rounded-lg transition-all text-sm font-medium"
                  >
                    {link.name}
                  </Link>
                ))}
                ...
                <div>
                  <button
                    ...
                    className="flex items-center justify-between w-full px-4 py-3 text-white/80 hover:text-[#C8F200] rounded-lg transition-all text-sm font-medium"
                  >
                    Services
                    <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${servicesOpen ? 'rotate-180' : ''}`} />
                  </button>
                      ...
                      >
                        {services.map((service) => (
                          <Link
                            key={service.name}
                            to={service.url}
                            className="block px-4 py-2.5 text-white/60 hover:text-[#C8F200] transition-all text-sm"
                          >
                            {service.name}
                          </Link>
                        ))}
